function handleDownloadClick(event) {
    ...
    const name = nameElement.textContent.trim(); // Extract and trim the name
    console.log("Name extracted:", name); // Debugging log

    // Check the status of the checkboxes
    const gcDownloadChecked = talkCard.querySelector('#gc_download').checked;
    const byuDownloadChecked = talkCard.querySelector('#byu_download').checked;

    let endpoint; // Define the Flask endpoint to call

    // Determine which endpoint to call based on the checkboxes
    if (gcDownloadChecked && byuDownloadChecked) {
        endpoint = 'http://127.0.0.1:5000/gc_byu_download'; // Endpoint for both GC and BYU downloads
    } else if (gcDownloadChecked) {
        endpoint = 'http://127.0.0.1:5000/gc_download'; // Endpoint for GC downloads
    } else if (byuDownloadChecked) {
        endpoint = 'http://127.0.0.1:5000/byu_download'; // Endpoint for BYU downloads
    } else {
