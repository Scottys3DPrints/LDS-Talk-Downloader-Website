...
function loadCurrentMembers() {
    fetch('json/current_with_byu.json') // Adjusted path to current_with_byu.json
...
function loadAlphabeticalMembers() {
    fetch('json/all_GAs_with_BYU.json') // Fetch from your specified JSON file
        .then(response => response.json())
        .then(data => {
            data.sort((a, b) => {
...
function displayMembers(members, showImages) {
    ...
    members.forEach(member => {
        ...
        if (showImages) {
            ...
            img.src = member.image || ''; // Assuming member.image holds the image URL
