...
function loadCurrentMembers() {
    fetch('json/current.json')
        ...
        .then(data => {
            displayMembers(data, true); // Show images
        })
...
function loadAlphabeticalMembers() {
    fetch('json/all_GAs.json')
        ...
        .then(data => {
            ...
            displayMembers(data, false); // Hide images
        })
...
function displayMembers(members, showImages) {
    ...
    members.forEach(member => {
        ...
        talkCard.className = 'talk-card flex_row';
        ...
        speakerInfo.className = 'flex_column flex_speaker';
        ...
        // Add other details (General Conference and BYU stats, for example)
        const gcInfo = document.createElement('div');
        gcInfo.className = 'flex_column flex_gc_byu';
        gcInfo.innerHTML = `
        ...
        byuInfo.className = 'flex_column flex_gc_byu';
        byuInfo.innerHTML = `
            <h4>BYU</h4>
            <p>Amount</p>
        `;
        ...
        downloadContainer.className = 'flex_column flex_download';
